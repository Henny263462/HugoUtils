...
class BlockOverlayConfigPage : ConfigPage {
    override val category = ConfigCategory.BLOCK_OVERLAY
    private val client = MinecraftClient.getInstance()
    private val picker by lazy { ColorPicker(client.textRenderer) { save() } }
    private val blocks by lazy { BlockPickerPanel(client.textRenderer) { save() } }
    ...
    override fun layout(x: Int, y: Int, width: Int, height: Int): Int {
        ...
        if (current == null || current !in BlockOverlayConfig.entries) {
    ...
    override fun mouseClicked(mouseX: Double, mouseY: Double): Boolean {
        if (toggle.contains(mouseX, mouseY)) {
            BlockOverlayConfig.enabled = !BlockOverlayConfig.enabled
            save()
            return true
        }
